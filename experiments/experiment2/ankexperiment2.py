...
def create_device_map(device_str):
    """
    Creates a device map based on the device string.
    Args:
        device_str: Either "auto" or comma-separated GPU IDs like "0,1,2,3,4,5,6,7"
    Returns:
    ...
    try:
        ...
        # Validate that the specified GPUs are available
        if not torch.cuda.is_available():
            print("Warning: CUDA is not available, falling back to 'auto'")
            return "auto"
        
        available_gpus = torch.cuda.device_count()
        for device_id in device_ids:
            if device_id >= available_gpus:
                print(f"Warning: GPU {device_id} is not available (only {available_gpus} GPUs found), falling back to 'auto'")
                return "auto"
        
        if len(device_ids) == 1:
            return f"cuda:{device_ids[0]}"
        else:
            # For multi-GPU setups (including 8 GPUs), use accelerate's automatic device mapping
            # The CUDA_VISIBLE_DEVICES environment variable will constrain it to specified devices
            print(f"Using multi-GPU setup with {len(device_ids)} devices: {device_ids}")
            print("Note: Model layers will be automatically distributed across specified GPUs")
            return "auto"  # Let accelerate handle the optimal distribution
            
    except ValueError:
        print(f"Warning: Invalid device specification '{device_str}', falling back to 'auto'")
        return "auto"

def quantize_4bit(model_path, device_map="auto"):
    """
    Load model with 4-bit quantization.
    For multi-GPU setups, the model will be automatically distributed across available GPUs.
    """
    return AutoModelForCausalLM.from_pretrained(
        ...
        # For large multi-GPU setups, enable memory efficient attention
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="auto",
        help="GPU devices to use (e.g. '0,1,2' or 'auto' for automatic allocation)"
    )
    ...
    if args.device != "auto":
        try:
            device_ids = [d.strip() for d in args.device.split(',')]
            os.environ['CUDA_VISIBLE_DEVICES'] = ','.join(device_ids)
            print(f"Set CUDA_VISIBLE_DEVICES to: {os.environ['CUDA_VISIBLE_DEVICES']}")
            print(f"Number of GPUs specified: {len(device_ids)}")
            
            # For large multi-GPU setups, provide additional info
            if len(device_ids) >= 4:
                print(f"Large multi-GPU setup detected ({len(device_ids)} GPUs).")
                print("Model layers will be automatically distributed across all specified GPUs.")
                if len(device_ids) == 8:
                    print("8-GPU setup: This should provide excellent performance for large models!")
        except:
            print(f"Warning: Could not set CUDA_VISIBLE_DEVICES for device specification: {args.device}")
    ...
    # Create device map based on user specification
    device_map = create_device_map(args.device)
    print(f"Using device map: {device_map}")
    
    model = quantize_4bit(args.model, device_map)
    model.eval()
